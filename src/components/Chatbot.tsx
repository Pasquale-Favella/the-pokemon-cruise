  ...
  return (
    <>
      <Button
        variant="outline"
        size="icon"
        className="fixed bottom-5 right-5 z-[1001] rounded-full p-6 shadow-lg transition-transform hover:scale-110 text-blue-500"
      ...
      {isOpen && (
        <Card className="fixed bottom-20 right-5 z-[1000] w-[350px] h-[450px] flex flex-col border border-gray-300 rounded-lg shadow-xl overflow-hidden bg-white py-0">
          <CardHeader className="bg-blue-600 text-white flex flex-row items-center justify-between px-4 py-3"> {/* Added padding here */}
            ...
            <ScrollArea className="h-full pr-4">
              {messages.map((msg, index) => (
                <div key={index} className={`flex items-start mb-4 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                  ...
                  <div className={`rounded-lg p-3 max-w-[80%] break-words ${msg.sender === 'user' ? 'bg-blue-500 text-white ml-auto' : 'bg-gray-200 text-gray-800'}`}>
                    {msg.text}
                  </div>
                  {msg.sender === 'user' && <User size={20} className="ml-2 text-blue-600" />}
                </div>
              ))}
              {isLoading && (
          ...
          <CardFooter className="flex bg-gray-100 p-4"> {/* Added padding here */}
            <Input
              ...
              className="flex-grow mr-2 rounded-md border border-gray-300 p-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <Button onClick={handleSend} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Send</Button>
          </CardFooter>
        </Card>
      )}
    </>
